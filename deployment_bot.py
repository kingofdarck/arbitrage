#!/usr/bin/env python3
"""
Telegram бот для деплоя - максимально надежный
Принудительно устанавливает кнопки при каждом взаимодействии
"""
...
def get_main_keyboard():
    """Получить основную клавиатуру - ПРИНУДИТЕЛЬНО"""
    keyboard = [
...
async def force_keyboard(update: Update, text: str):
    ...
    try:
        ...
        logger.error(f"❌ Ошибка установки клавиатуры: {e}")
        # Пробуем без markdown
        try:
            await update.message.reply_text(text, reply_markup=keyboard)
        except Exception as e2:
            logger.error(f"❌ Критическая ошибка клавиатуры: {e2}")
...
/settings - Настройки
    ...
    await force_keyboard(update, text)
...
async def show_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                f"💧 Ликвидность: {'✅' if settings['check_liquidity'] else '❌'}", 
...
💡 **Кнопки управления всегда внизу экрана**
    ...
    # Затем отправляем inline меню отдельным сообщением
    try:
        await update.message.reply_text(
            "🎛️ **Панель настроек:**", 
        ...
        logger.error(f"❌ Ошибка отправки inline меню: {e}")
...
def main():
    ...
    # Запускаем бота
    logger.info("🤖 Запуск надежного бота для деплоя...")
    print("🤖 Надежный бот запущен!")
    print("📱 Кнопки принудительно устанавливаются при каждом сообщении")
    print("💾 Настройки сохраняются в persistent_settings.json")
    print("🔧 Доступны команды: /start, /status, /settings")
